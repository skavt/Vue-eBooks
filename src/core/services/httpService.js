...
function handleExceptionResponse(response, resolveObj) {
	...
	if (response.status === HTTP_UNAUTHORIZED) {
		...
		resolveObj.toastError = i18n.t('Requested method is not allowed.');
		...
		resolveObj.toastError = i18n.t('Requested data was not found.');
	} else if (response.status === HTTP_INTERNAL_SERVER_ERROR) {
		resolveObj.toastError = response.message || i18n.t('Internal server error.');
	} else if (response.status === HTTP_FORBIDDEN) {
		resolveObj.toastError = i18n.t('You don\'t have permission.');
	} else {
		resolveObj.toastError = i18n.t('Something went wrong.');
	}
